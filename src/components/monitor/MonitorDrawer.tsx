  ...
  return (
    <div className="border-b border-border/60">
      <button
        ...
        className="flex items-center gap-2 w-full h-8 px-4 text-left hover:bg-muted/50 transition-all duration-150 cursor-pointer group"
      >
        <span className="text-muted-foreground group-hover:text-foreground transition-colors duration-150">{icon}</span>
        <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider flex-1">{title}</span>
        {open ? (
          <ChevronDown size={13} className="text-muted-foreground/60 transition-transform duration-200" />
        ) : (
          <ChevronRight size={13} className="text-muted-foreground/60 transition-transform duration-200" />
        )}
      </button>
        ...
        {isConnected ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-3 px-6 text-center">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-muted-foreground/60 leading-relaxed">{t('monitor.noConnectionHint')}</p>
            </div>
          </div>
        )}
      </div>
