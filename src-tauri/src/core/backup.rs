...
pub fn backup_data(destination: &str) -> Result<(), String> {
    ...
    log::info!("[backup] completed: {} ({} hosts, {} keys)",
        destination, payload.hosts.len(), payload.keys.len());
    Ok(())
}
...
pub fn restore_data(source: &str) -> Result<(), String> {
    ...
    log::info!("[restore] completed: {} hosts, {} keys, {} config entries",
        host_count, key_count, config_count);
    Ok(())
}
